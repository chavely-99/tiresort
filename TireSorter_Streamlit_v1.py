...
def load_tire_data_from_two_files(csv_file_source, excel_file_source) -> Tuple[pd.DataFrame, Optional[List[dict]]]:
    ...
    for set_num in sorted(csv_df['Set'].unique()):
        ...
        metrics = compute_set_metrics(
            corners['lf_data'], corners['rf_data'],
            corners['lr_data'], corners['rr_data']
        )
...
def validate_existing_sets(existing_sets: List[dict], all_tire_df: pd.DataFrame) -> Tuple[bool, List[str]]:
    """Validate that existing sets match the imported tire data.

    Validation checks:
    - Tire Numbers from existing sets exist in imported data
    - Rollout/Rate values match within tolerance (sanity check)
    - No duplicate tire Numbers across sets

    Returns:
        (is_valid, warnings):
            - is_valid: True if no hard errors (soft warnings ok)
            - warnings: List of validation messages (⚠️ warnings, ❌ errors)
    """
    warnings = []
    has_errors = False

    # Extract tire numbers from existing sets
    existing_numbers = set()
    for idx, s in enumerate(existing_sets):
        for corner in ['lf_data', 'rf_data', 'lr_data', 'rr_data']:
            tire = s[corner]
            if 'Number' in tire.index:
                num = int(tire['Number'])
                if num in existing_numbers:
                    warnings.append(f"❌ Tire #{num} appears in multiple existing sets")
                    has_errors = True
                existing_numbers.add(num)

    # Check that all existing numbers exist in imported data
    imported_numbers = set(all_tire_df['Number'].dropna().astype(int))
    missing = existing_numbers - imported_numbers
    if missing:
        warnings.append(f"❌ Existing set tires not found in import: {sorted(missing)}")
        has_errors = True

    # Sanity check: compare rollout/rate for existing tires (soft warnings)
    for idx, s in enumerate(existing_sets):
        for corner in ['lf_data', 'rf_data', 'lr_data', 'rr_data']:
            tire = s[corner]
            if 'Number' not in tire.index:
                continue
            num = int(tire['Number'])

            # Find matching tire in imported data
            matching = all_tire_df[all_tire_df['Number'] == num]
            if len(matching) == 0:
                continue

            imported_tire = matching.iloc[0]

            # Check rollout matches (within 1.0 tolerance - rescans can vary slightly)
            rollout_diff = abs(tire['Rollout/Dia'] - imported_tire['Rollout/Dia'])
            if rollout_diff > 1.0:
                warnings.append(
                    f"⚠️ Set {idx+1} tire #{num}: Rollout mismatch "
                    f"(expected {tire['Rollout/Dia']:.1f}, got {imported_tire['Rollout/Dia']:.1f}, diff={rollout_diff:.1f})"
                )

            # Check rate matches (within 10 lb tolerance)
            rate_diff = abs(tire['Rate'] - imported_tire['Rate'])
            if rate_diff > 10:
                warnings.append(
                    f"⚠️ Set {idx+1} tire #{num}: Rate mismatch "
                    f"(expected {tire['Rate']:.0f}, got {imported_tire['Rate']:.0f}, diff={rate_diff:.0f})"
                )

    return not has_errors, warnings


def filter_new_tires(all_tire_df: pd.DataFrame, existing_sets: List[dict]) -> pd.DataFrame:
    """Remove tires already in existing sets, return only new tires for sorting."""
    existing_numbers = set()
    for s in existing_sets:
        for corner in ['lf_data', 'rf_data', 'lr_data', 'rr_data']:
            if 'Number' in s[corner].index:
                existing_numbers.add(int(s[corner]['Number']))

    new_tires = all_tire_df[~all_tire_df['Number'].isin(existing_numbers)]
    return new_tires.reset_index(drop=True)
...
def refine_cross(solution: List[dict], cross_target: float, stagger_tolerance: float, road_course: bool = False) -> Tuple[bool, str]:
    ...
    # Filter out swaps involving locked sets
    locked_sets = st.session_state.get('locked_sets', set())
    swaps = [
        (a, ca, b, cb) for (a, ca, b, cb) in swaps
        if a not in locked_sets and b not in locked_sets
    ]
...
with tab_settings:
    ...
    with left_setup_col:
        ...
        import_mode = st.radio(
            "Import Mode",
            options=['Initial Import', 'Add Sets to Existing'],
            index=0 if st.session_state.import_mode == 'Initial Import' else 1,
            horizontal=True,
            help="**Initial Import**: Sort all tires from scratch. **Add Sets**: Validate existing sorted sets and sort only new tires."
        )
        st.session_state.import_mode = import_mode

        # Warning if changing to Initial Import with existing results
        if import_mode == 'Initial Import' and st.session_state.get('results') and len(st.session_state.results) > 0:
            st.warning("⚠️ You have existing sorted sets. 'Initial Import' will replace them. To add more sets, use 'Add Sets to Existing'.")
            if not st.checkbox("I understand, replace existing sets", key="confirm_replace"):
                st.stop()
        ...
        if both_files_uploaded:
            ...
            if st.session_state._upload_token != token:
                try:
                    ...
                    is_valid, warnings = validate_existing_sets(existing_sets, all_tire_df)

                    if not is_valid:
                        st.error("❌ Validation failed - cannot proceed:")
                        for w in warnings:
                            if '❌' in w:
                                st.error(w)
                        st.stop()

                    # Show warnings (soft issues)
                    if warnings:
                        for w in warnings:
                            if '⚠️' in w:
                                st.warning(w)
                    ...
                    # Store existing sets and mark as locked
                    st.session_state.results = existing_sets
                    st.session_state.locked_sets = set(range(len(existing_sets)))

                    # Extract existing tire numbers for reference
                    existing_numbers = set()
                    for s in existing_sets:
                        for corner in ['lf_data', 'rf_data', 'lr_data', 'rr_data']:
                            if 'Number' in s[corner].index:
                                existing_numbers.add(int(s[corner]['Number']))
                    st.session_state.existing_tire_numbers = existing_numbers

                    # Set tire_df to NEW TIRES ONLY for sorting
                    df = new_tires_df
    ...
    with right_setup_col:
        if st.session_state.data_loaded:
            ...
            if left is not None and right is not None and len(left) > 0 and len(right) > 0:
                ...
                # Check if enough new tires to make sets (Add Sets mode validation)
                if st.session_state.import_mode == 'Add Sets to Existing':
                    new_tire_count = len(left) + len(right)
                    if new_tire_count < 4:
                        st.warning(f"⚠️ Need at least 4 new tires to create a set. Currently have: {new_tire_count}")
            ...
            if run_sort:
                ...
                if not candidates:
                    ...
                    if solution:
                        ...
                        # If Add Sets mode, append new sets to existing
                        if st.session_state.import_mode == 'Add Sets to Existing':
                            existing_sets = st.session_state.results if st.session_state.results else []

                            # Combine: existing (locked) + new (unlocked)
                            combined_solution = existing_sets + solution
                            st.session_state.results = combined_solution

                            # Calculate stats for ALL sets (existing + new)
                            staggers = [s['stagger'] for s in combined_solution]
                            crosses = [s['cross'] for s in combined_solution]
                            st.session_state.stats = {
                                'n_sets': len(combined_solution),
                            ...
                            status_text.text(f"Done — Created {len(solution)} new sets (Sets {len(existing_sets)+1}-{len(combined_solution)}). Total: {len(combined_solution)} sets")
                            st.success(f"✅ Created {len(solution)} new sets. Total: {len(combined_solution)} sets")
                        else:
                            # Initial import: replace results entirely
...
with tab_results:
    if st.session_state.results is not None:
        ...
        def do_swap(from_set, from_corner, to_set, to_corner):
            """Swap individual tire between sets (with lock validation)."""
            # Prevent swaps involving locked sets
            locked_sets = st.session_state.get('locked_sets', set())

            if from_set in locked_sets:
                st.toast(f"❌ Cannot swap from Set {from_set+1} - it is locked", icon="🔒")
                return
            if to_set in locked_sets:
                st.toast(f"❌ Cannot swap to Set {to_set+1} - it is locked", icon="🔒")
                return
        ...
        def do_set_swap(set_a, set_b):
            """Swap entire sets (with lock validation)."""
            locked_sets = st.session_state.get('locked_sets', set())

            if set_a in locked_sets or set_b in locked_sets:
                st.toast(f"❌ Cannot swap locked sets", icon="🔒")
                return
        ...
        if row1_cols > 0:
            ...
            for i in range(row1_cols):
                ...
                with cols_row1[i]:
                    ...
                    is_locked = set_idx in st.session_state.get('locked_sets', set())

                    with st.container(border=True):
                        # Lock badge for locked sets
                        if is_locked:
                            st.markdown(
                                '<div style="background:#FFC107;color:#000;font-weight:bold;'
                                'text-align:center;padding:3px;border-radius:4px;margin-bottom:4px;'
                                'font-size:12px;">🔒 LOCKED SET</div>',
                                unsafe_allow_html=True
                            )
